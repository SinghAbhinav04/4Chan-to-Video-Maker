...
def get_llm_settings():
    ...
    return {
        ...
        "router_host": config.get("router_host", "127.0.0.1"),
        "router_port": config.get("router_port", 3456),
    }
...
def _call_llm(prompt: str, model: str = None, retries: int = 3) -> dict | None:
    ...
    if provider == "gemini" and settings["gemini_key"]:
        ...
        # Use simple base URL without trailing slash; library adds /v1/messages
        client = anthropic.Anthropic(base_url=f"http://{host}:{port}", api_key="sk-no-key-required")
        ...
        for attempt in range(1, retries + 1):
            try:
                response = client.messages.create(
                    ...
                    system="You are a helpful assistant that ALWAYS returns valid JSON blocks.",
                    temperature=cfg_temp,
                    max_tokens=4096,
                )
                raw = response.content[0].text
                print(f"  [LLM] Router raw response: {raw[:200]}...") # Log start of response
                raw = re.sub(r"```(?:json)?\s*|\s*```", "", raw).strip()
                if not raw:
                    print(f"  [LLM] Router returned empty text (attempt {attempt}/{retries})")
                    continue
                return json.loads(raw)
            except Exception as e:
                print(f"  [LLM] Router Error (attempt {attempt}/{retries}): {e}")
            time.sleep(1)
        return None
    else:
        cfg_model = settings["model"]
...
def _call_llm_stream(prompt: str, model: str = None):
    ...
    if provider == "gemini" and settings["gemini_key"]:
        ...
        try:
            ...
            print(f"  [LLM] Router Streaming Error: {e}")
            raise Exception(f"Router Streaming Error: {e}")
    else:
        cfg_model = settings["model"]
        model = model or cfg_model
        try:
...
Return ONLY valid JSON: {{"scores": [{{"id": <int>, "score": <float>, "hook": "<summary>"}}]}}"""
    ...
    try:
        ...
        if not raw_p1:
            yield ("error", "LLM returned empty response for Pass 1 Deep Analysis.")
            return

        print(f"  [Scout] Pass 1 raw text: {raw_p1[:100]}...")
        data_p1 = json.loads(raw_p1)
        scores = data_p1.get("scores", [])
        if not scores:
            yield ("error", "Failed to score threads (empty scores list).")
            return
...
Return valid JSON: {{"selected_thread_id": <int>, "reason": "<string>", "preview": "<string>"}}"""
        ...
        if not raw_p2:
            yield ("error", "LLM returned empty response for Pass 2 Selection.")
            return

        print(f"  [Scout] Pass 2 raw text: {raw_p2[:100]}...")
